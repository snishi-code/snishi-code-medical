...
## データ操作は fail-closed（失敗を握らない）— 臨床データの根本原則

患者データの**保存・削除・移動など「壊れたら臨床被害になる」操作は、失敗時に中断して明示通知**する。成功扱いで先へ進めない（fail-open 禁止）。「サイレントなデータ消失・取り違え・PII 残存」はすべてこの原則の欠如から生まれる。

- **catch で握って続行しない**: 永続化／削除／移動の `catch` は (a) 再 throw / (b) 成功・失敗を返す / (c) ユーザー通知して操作中断、のいずれか。`console.error` だけで先へ進まない。
- **可視状態を durable 状態より先に進めない**: 「完了」表示・完了 marker・画面切替・reload は、下層の保存／削除が**確認できてから**行う。
- **多段操作は atomic か補償付き**: 移動など複数ステップは「全部成功か全部無し」。途中失敗は補償（`captureSnapshot` での rollback 等）か明示失敗にする。
- **「無し」と「失敗」を区別**: 件数だけ返さず `{ ok, count }` 等で返す（0 件成功と失敗を取り違えない）。
- **削除は確認まで retryable**: PII 消去等は best-effort で捨てず、完了確認 or retry 可能な tombstone で追跡する。
- **保存系は throw する版を用意**: fire-and-forget の `saveNow()`（toast のみ・例外を投げない）とは別に、切替／移動が使う「失敗を返す／投げる」保存経路を持ち、呼び出し側で必ず処理する。IDB 不可（db=null）の no-op 保存も「保存できていない事実」として扱う。
...
- 展開フォーマットカードの値はポップアップでなく**その場 inline 編集 + 自動保存**（`formats.js` の `_inlineEdit`。値セルタップ→`.formatCardItem.editing`、input ごとに write-through 保存。保存/キャンセルボタンは無い。QR 等の再描画は編集終了時にまとめて）。クイック chip / ☰ ランチャー経由は従来どおり入力シート popup（`openFormatSheet`、明示保存/キャンセル）。
