...
- **8.12.0**: 老眼・スマホ不慣れ対応の UX 大改修（Phase 1〜4）＋ QR 基盤統一
  - **Phase 1 — 老眼・スマホ不慣れ対応**: 1列ホーム／大ボタン／追加導線／2段リスト。誤タップ削減・スマホ最適化、ホーム患者リストを一度のスワイプで最下部まで到達
  - **Phase 2 — 患者ライフサイクル**: 削除済み病棟／転棟／復元／完全削除／30日 purge。多段操作は fail-closed（atomic か補償付き）
  - **Phase 3 — タップ中心の S/O/A/P 入力**: 既定カード常設。`payload.buildPanelOut` の未タップ欄への既定文自動補完（QR fallback）を撤去し、タップした内容のみ QR に出力（不可逆 migration なし・自由記述互換は温存）
  - **Phase 4 — QR カメラ運用へ文言整理**: 受信導線／患者画面 QR 説明／アプリ内ヘルプ
  - **QR 基盤統一**: 設定／セット／フォーマットを同一基盤（`createQrFlow`）＋統一受信ルーター（`qr-receive.js`）に集約。取込保存を fail-closed 化。フォーマット項目／タグの並び替え UI
  - **起動前説明ページ**（`hospital-rounds-about.html`）を追加
  - E2E（Playwright）を現行 UI に追随＋操作契約を `helpers.js` へ集約
  - データ互換: wire/schema/bundle の bump なし・不可逆 migration なし＝**非破壊**（MINOR）

- **8.11.1**: PWA「削除して開始」を完全初期化に＋デッドコード削除（PWA 不変性=自動更新無効のセキュリティ意図を明文化）。move-patient／snapshot restore の単体テスト追加（fake-indexeddb）

- **8.11.0**: プライバシー強化＋保存失敗の可視化
  - ユーザー／病棟削除時にスナップショット DB の患者 PII を purge
  - 保存失敗をユーザーに可視化（サイレントなデータ損失を防ぐ）。iOS 入力ズーム防止・一括クリアの再描画漏れ修正

- **8.10.0**: ユーザー機能（案B）＋スナップショット復元＋研究ログ＋受信ボックス永続化＋プロブレムリスト改称
  - **ユーザー機能（1端末を複数人で切替）**: 予約レコード `__users__` ＋ `__settings__::<userId>` ＋ 病棟レコードに `userId`。ヘッダーのタイトル枠＝ユーザー名（タップ→user-picker）。設定はユーザー別。同名は1端末1人（重複拒否）。パスワードは `passhash:null` の器のみ。起動時に冪等 backfill。端末まるごと（全ユーザー）バックアップを追加
  - **巻き戻し（スナップショット）**: 独立DB `hospital-rounds-snapshots`。破壊操作（クリア/移動/取込）＋画面遷移の直前に自動撮影。病棟ごと first-of-day dedup＋nav直近2枚、14日失効。履歴保持型の復元
  - **研究用イベントログ**: 独立DB `hospital-rounds-eventlog`。無記名 `{t,u,k}`（患者名なし）、外部送信ゼロ、365日ローリング削除
  - **受信ボックス**（旧・受信メモ）: メモ/共有の受信を「常に受信ボックスに表示するだけ」に統一（マッチング反映・上書きを廃止）。病棟単位で永続化（bundle meta `recvMemo`/`recvShared`）。文字ボタン「消去する」「閉じる」＋「受信ボックスを開く」
  - **メモ → プロブレムリスト** に呼称統一（i18n値のみ。変数名・内部kindは据え置き）
  - 用語整理（削除＝オブジェクト／クリア＝選択・日次リセット／消去＝蓄積データ恒久削除）。デッドコード掃除（端末タイトル・旧ソートUndoのi18n等）
  - データ互換: 追加のみで破壊的変更なし（旧 bundle は recvMemo 欠落時 "" 既定、新DBは別ストア）
  - ※ 8.1〜8.9 は段階的に適用したが個別タグ/履歴記録を残していなかった（内容は `CLAUDE.md` 参照）。本バージョンから正式なタグ運用を再開する
...
- **7.6.1**: WS 名のヘッダー編集を復活 + 設定画面のラベルを DB アイコン化
  - **タイトル編集モード時に WS 名も editable に**: 鉛筆をタップ → タイトル・WS 名
    ...
    二系統並立 (設定画面側は複数 WS をまとめて編集する用途に維持)
  - **設定画面「ワークスペース管理」カードのラベルを DB アイコンに**: 他のカード
    (タグ・クリア対象 等) と同じく文字ではなく SVG アイコンで統一。i18n キーは
    title/aria-label として保持
