...
import { boot, goToHome, goToSettings, openPatient, startInlineEdit, inlineEditSet, endInlineEdit } from "./helpers.js";
...
test("手入力済みのチェックは上書きせず inline 編集を開き手入力を保持 (General)", async ({ page }) => {
  ...
  await inlineEditSet(page, firstRow.locator(".formatCardValue"), { value: "やや倦怠感あり" });
...
test("手入力で正常文と同一文字列にしてもタップで消えず inline 編集が開く (Phase6 provenance)", async ({ page }) => {
  ...
  await inlineEditSet(page, firstRow.locator(".formatCardValue"), { value: "良好" });
...
test("inline 編集: 既存値を別の値に変えて保存すると新しい値が残る (P1 回帰)", async ({ page }) => {
  ...
  await inlineEditSet(page, vitals.locator(".formatCardValue").nth(2), { value: "88" });
