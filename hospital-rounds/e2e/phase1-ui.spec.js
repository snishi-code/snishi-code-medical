...
// 2段表示は memo / shared とも本文が読み取り表示 (.memoRowBody, Phase 7 で一覧は読み取り化)。両方を実測する。
...
test("スマホ幅: プロブレムリストで患者ボタンと本文が2段に並ぶ", async ({ page }) => {
  ...
  await boot(page);
  await goToMemo(page);
  await assert2RowStacked(page, "#memoListHost", ".memoRowBody");
});
...
test("スマホ幅: 共有で患者ボタンと本文が2段に並ぶ", async ({ page }) => {
  ...
  await boot(page);
  await goToShared(page);
  await assert2RowStacked(page, "#sharedListHost", ".memoRowBody");
});
