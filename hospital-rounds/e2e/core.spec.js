...
import { boot, addUser, waitUserCount, waitRecvPersisted, goToMemo, goToSettings, goToHome, openPatient, setPatientStatus } from "./helpers.js";
...
test("受信ボックスはユーザーごとに分離され、リロードしても残る", async ({ page }) => {
  ...
  // 保存 (180ms debounce) が IDB に確定してから reload する。確定前に reload すると
  // beforeunload フラッシュが間に合わず値が消える (CI の低速 chromium で顕在化)。
  await waitRecvPersisted(page, "受信テストA");
...
test("2人以上いて1日経つと、起動時にユーザー選択が出る", async ({ page }) => {
  ...
  await expect(page.locator("#appTitleInput")).toHaveValue("田中"); // 切替完了を待つ
  // reload 後の起動ゲートは __users__ レコードを読んで「2人以上か」を判定する。
  // 田中の作成書き込みが durable になる前に reload すると CI (低速 chromium) では
  // ユーザー数が 1 のままに見えてユーザー選択が出ない (race)。IDB の確定を待ってから reload。
  await waitUserCount(page, 2);
  // 「最後の確認は2日前」にして再読込
  await page.evaluate(() =>
